 ...
 */
var Engine = function (global)  { //方法被调用时，传入this，即当前的窗体实例

    /* 实现定义我们会在这个作用于用到的变量
     * 创建 canvas 元素，拿到对应的 2D 上下文
     * 设置 canvas 元素的高/宽 然后添加到dom中
     */
    var doc = global.document, //获取当前页面的html文本
        win = global.window,
        // win = global,
    ...
    function drawInfo(content) {
        ...
        ctx.strokeText(content, 252, 303);

        ctx.font = "20pt Impact";

        ctx.strokeText('You got ' + player.gemNum * GEM_VALUE + ' score', 252, 353);


        ctx.font = "15pt Impact";
        ctx.strokeText('press space to restart...',252,400);
    ...
    function checkWin() {
       status = (!key.visible && gate.x === player.x && gate.y === player.y)? 'win': status;
    }
    ...
    function checkCollisions() {
        ...
        for(var i = 0; i < allEnemies.length; i++){
            ...
            if(Math.round(allEnemies[i].x) === player.x && Math.round(allEnemies[i].y) === player.y){
    ...
    function closeEngine() {
        ...
        form.onsubmit = function () {
            ...
            charValue = init_form.character.value;

            tollGateIndex = parseInt(init_form.select.value);
    ...
    global.closeEngine = closeEngine;
